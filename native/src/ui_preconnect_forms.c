...
clog_define(g_native_log_ui_forms, cLogLevelInfo, cLogFlags_Default, "ui.forms", NULL);
...
bool ui_profile_dirty(const NativePreconnectUi *ui, int slot) {
    ...
    return strcmp(draft->name, saved->name) != 0 || strcmp(draft->host, saved->host) != 0 ||
           ...
           draft->camera_redirect != saved->camera_redirect ||
           draft->audio_input_redirect != saved->audio_input_redirect ||
           ui_current_audio_codec(ui) != ui->committed_audio_codec;
}

void native_ui_preconnect_forms_ready(void) {
    clog(cLogLevelTrace, "pre-connect forms ready");
}
